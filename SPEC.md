...
```
  ...
  feeds ──────────► │  EventBridge Scheduler ──► Lambda poller (1 per src)  │
                    ...
                    └───────────────────────┬────────────────────────────────┘
                                            ▼
                         static HTML + email, 16:00 Africa/Casablanca
                                            │
...
- **Capacity, stated plainly** — enrichment runs against **cluster heads once per pre-brief window,
  ...
  loudly rather than silently lagging the 16:00 send.
...
| **4A** | same | §7.4's full component set | email, 16:00 |
| **4B** | + enrichment, + macro revisions | same | email, 16:00 |
...
|---|---|---|
| **Kafka** | A source arrives that is **genuinely continuous** rather than polled, **and** there is a second, independent consumer of `articles.normalized` that is not the batch clustering job — e.g. a live ticker-alert path with its own latency SLA — **measured 2026-08-29: 9 polled sources, 0 continuous; 9 readers of `silver.articles`, 0 with a latency SLA of their own (ADR-0015)** | **The sources are micro-batch by nature.** Feeds emit on a 5–30 minute cadence and are reached by polling, so there is no continuous upstream stream to preserve — a topic here would manufacture streaming semantics the data never had. It would also sit between one local Spark job and another, with `bronze/` already the replay source of truth. A topic with a single consumer and no process boundary is a résumé line you then have to defend |
...
| **dbt** | The gold layer exceeds ~10 models, or hand-written tests start duplicating each other — **measured 2026-08-29: 4 models, and no silver→gold SQL layer to migrate (ADR-0015)** | Below that, dbt's lineage and test framework cost more setup than they save |
| **pgvector** | The vector working set exceeds ~50k embeddings | Concretely: clustering holds a 72-hour window (a few thousand articles) and novelty scores against ~30 days of cluster heads (~1k–3k vectors). That is a numpy array and a cosine call, not a database. Standing up Postgres for 3k vectors is the kind of over-engineering the rest of this document exists to avoid. **Measured 2026-08-29: 1,888 + 9,379 = 11,267 — the cluster-head estimate above was low by ~3x; the gate is reached around 2026-12 at the current rate (ADR-0015)** |
| **Automated ranker weight fitting** | The feedback table holds several hundred marked items — **measured 2026-08-29: 1 mark across 60 items (ADR-0015)** | See §7.4 |
